...
export default function AssociateImg(props) {
  ...
  const listAllPhotos = () => {
    axios.get('/dashboard/listPhotos').then(res =>{
  ...
  const thumbsPhotosActual = actualPhotos.map((file,i) => (
    <div key={i}>
        <div style={thumb} >
            <div style={thumbInner} onClick={() => {
                  axios.delete(`/dashboard/products/${productId}/image/${file.id}`).then(res => {
                    //listAllPhotos()
  ...
  const saveSelectedPhotos = () => {
    // eslint-disable-next-line 
      ...
      selectedPhotos.map((file) => {
        axios.post(`/dashboard/${productId}/image/${file.id}`).then(() => {
            setSelectedPhotos([])
            handleClose()
            listPhotos()
            })
